#!/bin/bash
# app-relaunch.sh — Stop any running Vox menu-bar app, rebuild the dev binary, and relaunch it.
#
# Usage: ./scripts/app-relaunch.sh [--no-build] [--foreground]
#
# Options:
#   --no-build        Skip rebuild (relaunch existing dev binary)
#   --foreground, -f  Run in foreground (Ctrl-C to stop, see logs in terminal)
#                     Default is detached (logs at app/.logs/vox-dev.log)
...
ANY_PATTERN="(${INSTALLED_PATTERN}|${DEV_PATTERN})"
...
if $DETACH; then
  ...
  LOG="$ROOT/app/.logs/vox-dev.log"
  say "launching dev binary (detached)"
  nohup "$BIN" >"$LOG" 2>&1 &
  PID=$!
  disown
  ok "Vox relaunched (pid $PID)"
