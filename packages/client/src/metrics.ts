import type { AnnotationMetrics, SynthesisMetrics, TranscriptionMetrics } from "./types.ts";
...
export function parseAnnotationMetrics(
  value: unknown,
  fallbackElapsedMs: number,
): AnnotationMetrics | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }
  ...
  return {
    ...
    audioLoadMs: Number(raw.audioLoadMs ?? 0),
    audioPrepareMs: Number(raw.audioPrepareMs ?? 0),
    diarizationMs: Number(raw.diarizationMs ?? raw.inferenceMs ?? 0),
    totalMs: Number(raw.totalMs ?? fallbackElapsedMs),
    realtimeFactor: Number(raw.realtimeFactor ?? 0),
  };
}
